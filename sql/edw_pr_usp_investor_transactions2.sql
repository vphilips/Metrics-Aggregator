CREATE OR ALTER PROCEDURE edw.pr_usp_investor_transactions2
    (
        ...
        @ViewCurrencyCode   varchar(20)  = 'USD',     -- Replaced @CurrencyIDs list with single View Currency
...
BEGIN
    ...
    -- Fallback if invalid currency code passed (optional, sets to 1 or -1 or errors out)
    -- IF @ViewCurrencyID IS NULL SET @ViewCurrencyID = 1; -- Example default
...
LEFT JOIN edw.exchange_rates xr
  ...
  AND xr.Date = cal.calendar_date  -- Assuming Rate Date matches Transaction Date
...
GROUP BY
    f.Metric_ID,
    f.investor_name_id,
    -- Grouping by converted currency is constant for the view
    cal.calendar_date,
...
ORDER BY ';
    ...
    SET @sql += CASE @OrderBy
        ...
        WHEN 'amount'   THEN N' SUM(f.amount * ISNULL(xr.Rate, 1)) ' -- Sort by converted amount
